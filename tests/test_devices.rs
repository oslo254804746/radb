...
mod test_devices {
    ...
    fn test_iter_directory() {
        ...
        let mut file_info = device
            .iter_directory("/data/local/tmp")
            .unwrap()
    ...
    fn test_shell1() {
        let mut device = setup();
        let command = " ls /data/local/tmp -all";
        let output = device.shell(command).unwrap();
    ...
    fn test_shell3() {
        let mut device = setup();
        let command = vec!["ls", "/data/local/tmp", "-all"];
        let output = device.shell(&command).unwrap();
